...
const command: Command = {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    console.log(`[요약시작] deferReply 완료, 채널: ${voiceChannel.name} (${voiceChannel.id})`);
    ...
    connection.on('stateChange', (oldState, newState) => {
      console.log(`[요약시작] 연결 상태: ${oldState.status} → ${newState.status}`);
    });
    ...
    try {
      ...
      console.error('[요약시작] 연결 실패, 현재 상태:', connection.state.status, err);
      connection.destroy();
